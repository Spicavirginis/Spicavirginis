...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            wait: '[ WAIT ] Sedang di proses⏳ Tungguin Bntar okehh?',
            error: {
                St: '[❗] Kirim gambar dengan caption *!sticker* atau reply gambar yg udh dikirim tadi',
                Qm: '[❗] Terjadi kesalahan, mungkin themenya gak tersedia!',
                Yt3: '[❗] Terjadi kesalahan, tidak dapat meng konversi ke mp3!',
                Yt4: '[❗] Terjadi kesalahan, kemungkinan error dari sistem.',
                Ig: '[❗] Terjadi kesalahan, Akunnya di private kali tuh',
                Ki: '[❗] Njir, Bot gabisa ngekick admin group bro!',
                Ad: '[❗] Gabisa nambahin Target, Diprivate mungkin',
                Iv: '[❗] Link yang lu kirim gak valid bro!'
            }
        }
        ...
        const ownerNumber = ["628978878976@c.us","55xxxxx"] // replace with your whatsapp number
        ...
        case '!donasi':
        case '!donate':
            client.sendLinkWithAutoPreview(from, 'https://www.github.com/Spicavirginis/Spicavirginis', donate)
            break
        case '!tts':
            if (args.length === 1) return client.reply(from, 'Kirim perintah *!tts [id, en, jp, ar] [teks]*, contoh *!tts jp あなたはろくでなしをクソ*')
	    ...
	    const ttsJp = require('node-gtts')('ja')
            ...
            if (dataText.length > 500) return client.reply(from, 'Teks terlalu panjang! Kek peler Gwehh!', id)
        ...
        case '!ytmp3':
            ...
            try {
                ...
                if (resp.error) {
                    ...
                    if (Number(filesize.split(' MB')[0]) >= 30.00) return client.reply(from, 'Maaf, Durasinya Melampaui Batas maksimal :"(', id)
        ...
        case '!fb':
            ...
            break
        case '!creator':
            client.sendContact(from, '628978878976@c.us')
            break
        case '!ig':
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		client.reply(from, 'Maaf, Gatau gue itu anime apa', id)
                	}
        ...
        case '!linkgroup':
            ...
            if (isGroupMsg) {
            	...
            	client.reply(from, 'Perintahnya cuma bisa dipake dalam Group!', id)
            }
            break
        case '!bc':
            if (!isOwner) return client.reply(from, 'Perintah ini khusus buat owner!', id)
            let msg = body.slice(4)
            const chatz = await client.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendText(ids, `[ Spica Virginis BOT Broadcast ]\n\n${msg}`)
            }
            client.reply(from, 'Broadcast Success!', id)
            break
        case '!adminlist':
            if (!isGroupMsg) return client.reply(from, 'Perintahnya cuma bisa dipake dalam Group!', id)
            let mimin = ''
        ...
        case '!mentionall':
            if (!isGroupMsg) return client.reply(from, 'Perintahnya cuma bisa dipake dalam Group!', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintahnya cuma bisa dipake dalam Group', id)
        ...
        case '!join':
            ...
            if (check.size < minMem) return client.reply(from, 'Member groupnya gak melebihi 30, bot gabisa masuk', id)
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'Oke omw!'))
            } else {
        ...
        case '!randomhentai':
            if (isGroupMsg) {
                if (!isNsfw) return client.reply(from, 'Command/Perintah NSFW belum di aktifin di group ini bro!', id)
        ...
        case '!readme':
            ...
            break
        case '!info':
            client.sendLinkWithAutoPreview(from, 'Gagal Memuat Info untuk sekarang!', info)
            break
        case '!snk':
